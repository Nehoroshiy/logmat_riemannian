...
class Problem:
    ...
    def prepare(self, need_grad=False, need_hess=False):
        ...
        if self.cost is None:
            self._cost = tf.compile(self.theano_cost, self.theano_arg)
            if isinstance(self.theano_arg, list) or isinstance(self.theano_arg, tuple):
        ...
        if need_grad and self.grad is None:
            ...
            if self.egrad is None:
                self._egrad = tf.gradient(self.theano_cost, self.theano_arg)
                if isinstance(self.theano_arg, list) or isinstance(self.theano_arg, tuple):
                    self.egrad = lambda x: self._egrad(*x)
                else:
                    self.egrad = self._egrad
            
            if isinstance(self.theano_arg, list) or isinstance(self.theano_arg, tuple):
                self.grad = lambda x: self.man.from_partial(x, self.egrad(x))
            else:
