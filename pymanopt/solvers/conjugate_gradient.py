...
class ConjugateGradient(Solver):
    ...
    """
    def __init__(self, beta_type=BetaTypes.PolakRibiere, orth_value=np.inf,
                 *args, **kwargs):
    ...
    def solve(self, problem, x=None):
        ...
        desc_dir = man.lincomb(x, -1.0, Pgrad)

        while True:
            if self._verbosity >= 2:
                print((iter, cost, gradnorm))
            ...
            if abs(orth_grads) >= self._orth_value:
                beta = 0
                desc_dir = man.lincomb(newx, -1.0, Pnewgrad)
            else:
                ...
                if self._beta_type == BetaTypes.FletcherReeves:
                    ...
                    diff = man.lincomb(newx, 1.0, newgrad, -1.0, oldgrad)
                    ...
                    Pdiff = man.lincomb(newx, 1.0, Pnewgrad, -1.0, Poldgrad)
                ...
                desc_dir = man.lincomb(newx, -1.0, Pnewgrad, beta, desc_dir)
